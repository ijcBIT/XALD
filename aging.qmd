...
library(S4Vectors)
tar_load(filtered_inhouse)
tar_load(ss_clean_inhouse)
ss<-data.table::as.data.table(ss_clean_inhouse)
controls<-filtered_inhouse[,ss[Type=="Control",Sample_Name]]
```
...
library(data.table)
tar_load(ss_clean_inhouse)
ss_ctl<-ss_clean_inhouse[Type=="Control",]
tar_load(filtered_inhouse)
controls<-filtered_inhouse[,ss_ctl$Sample_Name]
dim(controls)
...
dt<-p_long[model=="sig_mod"&abs(diff)>0.05,.SD,.SDcols=c("ref","cg","type","diff")]

# 3. add gene name information to our data.table
data.table::setkey(dt,cg)
dt$gene_name <- EPIC_anno[dt$cg,UCSC_RefGene_Name]
```
...
```{r}
probelist$cgIn_consensus <- unique(dt$cg)#intersect(row.names(sig_mod), unique(unlist(probelist)))
```
...
anno<-dt[probelist$cgIn_consensus,]
...
# Store results 
cgIn_consensus$hypo <- pathways.hypo[[1]]
...
# Store results 
cgIn_consensus$hyper <- pathways.hyper[[1]]
...
```{r}
probelist$cgIn_top <- rownames(limma::topTable(full_mod,number=5000))
```
...
```{r}
probelist$cgIn_literature <- intersect(
  rownames(sig_mod),
  probelist$cgSet_pre
  )
```
...
library(targets)
library(data.table)
ss_adults<-ss_clean_inhouse[Age > 18, ]
targets::tar_load(filtered_inhouse)
adults<-filtered_inhouse[,ss_adults$Sample_Name]
dim(adults)
...
##### ~~A. Overall ammount of methylation in each contrast vs proportion of methylation for the probes in age signature sets~~
