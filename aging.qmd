...
dmp10<-dt22[order(-abs(Beta_difference)),head(.SD,10),by=Contrast]
dtable(dmp10)
```
...
```{r}
dtb <- as.data.table(betas,keep.rownames = "cg")
...
ggplot(betas_age["cg00821341",], aes(x = Age, y = bVal, color=Condition )) +
    geom_point() +
    stat_summary(
    geom = "point",
    fun.y = "mean",
    ...
    labs(title='Residual vs. Age', x='Age', y='Residuals')  
...
ggplot(pdata, aes(x = Age, y = bVal, color=Condition )) +
  geom_point() +
