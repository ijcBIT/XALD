...
![](analysis/aging/ctl/density_plot.png)

![beanplot](analysis/aging/ctl/bean_plot.png){alt="beanplot"}


There are `r dim(controls)[1]` probes that pass QC and `r dim(controls)[2]` control samples in our dataset.
...
1.  sig_mod: We could either intersect our significant probes `nrow(sig_mod)` with the sites that have been previously reported to be related with age.
...
There are `r dim(adults)[1]` probes that pass QC and `r dim(adults)[2]` control samples in our dataset.
...
Following the aforementioned paper [by Wilma, T. et al](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4082572/) we now normalize the data using the subset-quantile within array normalization (SWAN) method:
...
#### Model1: correct for age and use residuals:
...
`r nrow(sig_res_mod)` were significantly regulated (Lima *t* test, *p* \< 0.01) between the group of adult and child subjects using −2 \> *t* value \> 2 as a cut-off.
...
dtb_res <- as.data.table(betas,keep.rownames = "cg")
dtbm_res <- melt(dtb_res,variable.name = "Sample_Name", value.name = "bVal")
```
...
dtb_cov <- as.data.table(betas,keep.rownames = "cg")
dtbm_cov <- melt(dtb_cov,variable.name = "Sample_Name", value.name = "bVal")
```


```{r}
betas_desease<-c(betas_cov,betas_res)
pdata<-rbindlist(betas_desease)
...
```{r}
dtb <- as.data.table(pdata,keep.rownames = "cg")
dtbm <- melt(dtb,variable.name = "Sample_Name", value.name = "bVal")
dtbm<-dtbm[Condition != "Age",]
betas_age<-merge(dtbm,ss_adults[,.SD, .SDcols=c("Sample_Name", "Age","Condition")])
```
...
Here we inspect the top DMP sites individually.
...
topDMPs<-dmp10[dmp10[,probe] %in% betas_age$cg,]
...
plts<-lapply(
  ...
  function(ProbeID){
    dt_mean <- betas_age[ProbeID,list(b=mean(bVal),avage=mean(Age)),by=Condition]
    
    print(ProbeID)
    p<-ggplot(betas_age[ProbeID,], aes(x = Age, y = bVal, color=Condition )) +
    geom_point() +
...
First we will plot the distribution of the beta values for each contrast:

```{r, fig.height=42, dpi=200}
library(ggplot2)
library(viridis)
...
# 2. Filter for significant probes and absolute mean difference of beta values > 0.05 
dt_path<-pdata[model=="mod2_ext"&abs(diff)>0.05,.SD,.SDcols=c("ref","cg","type","diff")]
...
```{r}
top5 <- limma::topTable(mod2_ext,5000)
```

For convinience the chunk of code above has been converted to a function:

```{r}
pathways_desease <- get_pathway(probeIDs = top5,dt=dt_path)
...
setcolorder(ss_horvath,"SampleID")
b<-as.data.table(betas,keep.rownames = T)
b[,ProbeID:=rn]
b$rn<-NULL
setcolorder(b,c("ProbeID",ss_horvath$SampleID))
fwrite(b,"data/adult_betas.csv")
fwrite(ss_horvath,"data/ss_adults.csv")
```
