...
plts<-lapply(
  ...
  function(ProbeID){
    dt_mean <- pdata_indiv[ProbeID,list(b=mean(bVal),avage=mean(Age)),by=c("Condition","model")]
    
    print(ProbeID)
    p<-ggplot(pdata_indiv[ProbeID,], aes(x = Age, y = bVal, color=Condition, group=factor(model) )) +
    geom_point() +
...
#| label: tbl-adult.z-test cov model
...
#| label: tbl-adult.z-test residuals model
#| tbl-cap: "Adult Disease vs Adult Controls methylation distributions significance test."
