...
```{r top_probes_all, fig.height=72, dpi=200}
#| column: page
#| layout-ncol: 1
...
dt_overall <- dt10[Contrast != "Disease-Control",]
dtp_overall <-merge(dt_ind_cov,head(dt_overall[order(-abs(Beta_difference)),list(cg=probe, Contrast=Contrast)],20))
dt_mean2 <- dtp_overall[,list(b=mean(bVal),avage=mean(Age)),by=c("Condition","model","cg")]

p_overall<-ggplot(dtp_overall, aes(x = Age, y = bVal, color=Condition, group=factor(model) )) +
    geom_point() +
    ylim(0, 1) +
    geom_point(data=dt_mean2,aes(x=avage,y=b,col=Condition),size=4,shape=24)  +
    geom_smooth(aes(group=Condition),
              method = "lm", formula = y ~ x, se = F) +
